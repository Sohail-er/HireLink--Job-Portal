...
const NotiMenu = () => {
    ...
    return <Menu shadow="md" width={400} opened={opened} onChange={setOpened}>
        <Menu.Target>
            <div className=" bg-white/90 backdrop-blur-sm border border-purple-200 p-1.5 rounded-full">
                <Indicator disabled={notifications.length<=0} color="purple.6" offset={6} size={8} processing>
                    <IconBell stroke={1.5} />
                </Indicator>
            </div>
        </Menu.Target>
            ...
            <div className="flex flex-col gap-1">
                {
                    notifications.map((noti: any, index:number) => <Notification onClick={()=>{
                     ...
                     key={index} className="hover:bg-purple-50 cursor-pointer" onClose={()=>unread(index)} icon={<IconCheck  style={{ width: rem(20), height: rem(20) }} />} color="teal" title={noti.action} mt="md">
                        {noti.message}
                    </Notification>
)}
{
