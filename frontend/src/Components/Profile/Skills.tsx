...
const Skills=()=>{
    ...
    <div className="text-2xl font-semibold mb-3 flex justify-between text-gray-800">Skills <div>
    ...
    {edit ? <TagsInput data-aos="zoom-out"  placeholder="Add skill" value={skills} onChange={setSkills} splitChars={[',', ' ', '|']} />
        : <div className="flex flex-wrap gap-2">
            {
                profile?.skills?.map((skill:any, index:number) => <div key={index} className="bg-gradient-to-r from-purple-100 to-pink-100 rounded-3xl px-3 py-1 text-sm font-medium text-purple-600">{skill}</div>)
            }
        </div>}
