...
const Jobs = () => {
    ...
    return <div className="px-5 py-5">
        <div className="flex justify-between flex-wrap mt-5">
            <div className="text-2xl xs-mx:text-xl flex gap-3 items-center font-semibold text-gray-800">Recommended jobs   {Object.keys(filter).length>0&&<Button onClick={()=>dispatch(resetFilter())} className="font-body transition duration-300 !bg-gradient-to-r !from-purple-500 !to-pink-500 !text-white hover:!from-purple-600 hover:!to-pink-600 !border-0" size="compact-sm" leftSection={<IconX stroke={1.5} size={20}/>} variant="filled" >Clear Filters</Button>}</div>
            <Sort sort="job" />
        </div>
        <div className="flex mt-10 flex-wrap gap-5">
            {
                filteredJobs.length>0?filteredJobs.map((job:any, index:any) => <JobCard key={index} {...job} />):<div className="font-medium text-lg text-gray-600">
                    No job found
                </div>
            }
        </div>
